# CAVE Utils: Development Instructions
...
utils/
  prettify.py                  # autoflake + black formatter
  docs.py                      # Regenerate pdoc documentation
noxfile.py                     # nox sessions: runs pytest across Python 3.11–3.14
pyproject.toml                 # black config: line-length=100, target py311; pytest config
uv.lock                        # uv lockfile
```
...
| `uv run python utils/prettify.py` | Format with autoflake + black (line-length=100) |
| `uv run python utils/docs.py` | Regenerate pdoc documentation |

**Test runner details**: pytest collects every `*.py` file in `test/` (excluding `test/api_examples/`) and runs all `test_*` functions.

**`test_validator.py` specifically**: Dynamically imports every file in `test/api_examples/`, calls `execute_command(session_data={}, socket=Socket(silent=True), command="init")`, and asserts that `Validator(session_data=result).log.log == []` (empty log = no errors/warnings).
...
4. Update `VERSION` in `utils/docs.py` and run `uv run python utils/docs.py` to regenerate docs
5. Verify the generated `docs/` looks correct
